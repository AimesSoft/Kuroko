...
仍在进行中：生产级 HDR/EDR 输出、成熟的 tone mapping/scaler 质量、ICC/display profile 处理、更精确的 CoreAudio host-time/device-time 对齐、真实 libass 静态链接 renderer、原生弹幕 glyph atlas/batching、真正的 wgpu compositor 实现，以及打包好的 Flutter plugin glue。
