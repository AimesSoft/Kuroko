...
- 早期字幕解析、弹幕解析/布局，以及共享 text-measurement 边界。
- 基于 interleaved f32 PCM ring buffer 的 CoreAudio 输出。
- 带有有界视频/音频队列的 `Player` worker thread。
...
仍在进行中：生产级 HDR/EDR 输出、成熟的 tone mapping/scaler 质量、ICC/display profile 处理、audio-master A/V sync、libass bitmap rendering、原生弹幕 glyph atlas/batching、真正的 wgpu compositor 实现，以及打包好的 Flutter plugin glue。
...
当前 FFmpeg 构建路径已经启用。libass、HarfBuzz、FreeType 与 FriBidi 已由 `xtask` 固定版本并准备下载；完整静态链接路径会随字幕/text renderer milestone 落地。
