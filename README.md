...
Erika 是一个用 Rust 编写的独立媒体播放器引擎。

Erika 负责播放控制、时序同步、原生渲染、字幕、弹幕、音频输出，以及面向 HDR/EDR 的呈现路径。

当前首要实现目标是 macOS 14+ 与原生 Metal presenter。长期方向是优先使用各平台原生渲染器，并在原生 API 不适合的场景下提供 wgpu fallback。

## 当前状态

Erika 仍处于早期引擎基础阶段，还不是完整的终端播放器。当前代码树已经包含一条可运行的 macOS 播放路径：

- 通过 `xtask` 从源码构建 FFmpeg 7.1.1，默认 profile 面向 LGPL。
- 基于 Erika 本地文件与 HTTP Range source abstraction 的自定义 FFmpeg `AVIOContext`。
...
- 早期字幕解析、feature-gated libass 静态链接 renderer、libass-backed overlay timeline、弹幕解析/布局，以及共享 text-measurement 边界。
...
- 面向嵌入方的 `PresenterRuntime`，可由 Erika 接管 playback、Metal rendering、overlays 与 CoreAudio。
- `erika_capi` 提供 opaque C handles，可供 Rust、C/C++、Swift/ObjC，以及未来 Dart FFI 使用。
...
Erika 运行时不依赖 mpv、gpu-next 或 libplacebo。渲染方向是干净的 Rust 实现，但会参考同类现代媒体渲染器中的设计：显式 frame import、color state、shader graph、tone mapping、scaling、overlay composition、dithering 与 native presentation。

## 仓库结构

```text
crates/erika              主播放器引擎 crate
crates/erika_capi         Opaque-handle C ABI
crates/erika_ffmpeg_sys   低层 FFmpeg generated bindings
docs/                      架构与嵌入说明
...
Erika 使用源码构建的原生依赖，以便控制发布产物与许可证边界。默认 profile 是 `lgpl`；`gpl-full` 保留给未来可选构建。
...
当前 FFmpeg 构建路径已经启用。传入 `--all` 时，`xtask` 还会构建 libass、HarfBuzz、FreeType 与 FriBidi；`erika` 的 `libass` feature 会静态链接这些依赖并启用真实 ASS bitmap renderer。
...
cargo test -p erika --features libass
...
cargo run -p macos_native_demo --features libass -- --ass-subtitle "/path/to/subtitle.ass" --smoke-seconds 1.5 "$SAMPLE"
```
...
Flutter 现在已经可以通过 C ABI 调用 Erika 的 player lifetime、media open、playback commands、seeking、state queries、event polling 与 native surface attachment。Flutter plugin 层本身还没有打包完成。

当前有两组 C ABI entrypoint：

- `ErikaHandle`：控制/事件 API，适合由宿主管理呈现的场景。
- `ErikaPresenterHandle`：macOS presenter API，由 Erika 持有 `Player`、`MetalRenderer`、CoreAudio output 与 `render_tick()`。

macOS HDR 嵌入的推荐路径是原生 Metal-backed surface：Flutter widget 预留一个矩形区域，macOS plugin 托管原生 `CAMetalLayer`，Erika 通过 `erika_presenter_attach_metal_layer` 与 `erika_presenter_render_tick` 直接渲染到该 layer。
