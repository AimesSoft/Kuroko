...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=KUROKO_FFMPEG_DIR");
    ...
    if !include_dir.join("libavformat/avformat.h").exists() {
        panic!(
            "FFmpeg headers were not found at {}. Run `cargo run -p xtask -- deps build --profile {}` first, or set KUROKO_FFMPEG_DIR.",
            include_dir.display(),
            native_profile()
        );
    }
...
fn ffmpeg_dist_dir() -> PathBuf {
    if let Ok(path) = env::var("KUROKO_FFMPEG_DIR") {
