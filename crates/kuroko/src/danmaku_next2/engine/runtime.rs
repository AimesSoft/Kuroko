...
pub fn render_frame_bgra(
    ...
    input: RenderFrameInput,
    timeout: Duration,
) -> Option<Next2ReadbackFrame> {
    let entry = lookup_engine(handle)?;
    let (reply_tx, reply_rx) = mpsc::channel();
    entry
        .cmd_tx
        .send(EngineCommand::RenderFrame {
            input,
            reply: reply_tx,
        })
        .ok()?;
...
fn extract_mtl_device_ptr(device: &wgpu::Device) -> *mut std::ffi::c_void {
    let result = unsafe {
        device.as_hal::<wgpu_hal::api::Metal>().map(|hal_device| {
            ...
            objc2::rc::Retained::as_ptr(raw) as *mut std::ffi::c_void
        })
    };
    ...
    while running {
        ...
        loop {
            ...
            match cmd {
                ...
                EngineCommand::RenderFrame { input, reply } => {
                    let font_source = load_custom_font_source(
                        input.custom_font_family.as_str(),
                        input.custom_font_file_path.as_str(),
                    )
                    .ok()
                    .flatten();
                    let frame = if renderer.update_frame(input, font_source) {
                        renderer.draw_to_offscreen();
                        renderer.readback_pixels()
                    } else {
                        None
                    };
