...
#[cfg(target_os = "macos")]
use wgpu_hal::{CopyExtent, api::Metal};
...
pub(crate) fn attach_present_texture(
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let raw_ptr = mtl_texture_ptr as *mut std::ffi::c_void;
        if raw_ptr.is_null() {
            return None;
        }

        let raw_texture: Retained<ProtocolObject<dyn MTLTexture>> =
            unsafe { Retained::retain(raw_ptr.cast()) }?;
        ...
        let render_texture = unsafe { device.create_texture_from_hal::<Metal>(hal_texture, &desc) };
