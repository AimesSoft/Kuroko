...
pub fn next2_prepare_layout(
    ...
    for idx in 0..items.len() {
        ...
        let item_time = items[idx].time_seconds;
        {
            ...
            item.height = measure_text_height(font_size * item.font_size_multiplier);
...
pub fn next2_layout_frame_ref(
    layout: &RustNext2PreparedLayout,
    current_time_seconds: f64,
) -> RustNext2FrameLayout {
    build_next2_frame(layout, current_time_seconds)
}
...
struct RawNext2Item {
    ...
    color_argb: i32,
    is_me: bool,
    original_font_size_multiplier: f64,
...
impl RawNext2Item {
    fn from_plain(item: RustNext2DanmakuItem) -> Self {
        Self {
            ...
            is_me: item.is_me,
            original_font_size_multiplier: sanitize_font_size_multiplier(item.font_size_multiplier),
            font_size_multiplier: sanitize_font_size_multiplier(item.font_size_multiplier),
            count_text: None,
        }
    }
}
        ...
        if count > 1 {
            ...
            let first_raw = processed.get(&first_key).cloned().unwrap_or_else(|| {
                RawNext2Item::from_plain(RustNext2DanmakuItem {
                    ...
                    is_me: current.is_me,
                    font_size_multiplier: current.font_size_multiplier,
                })
            });

            let mut first_processed = first_raw.clone();
            first_processed.font_size_multiplier = sanitize_font_size_multiplier(
                first_processed.original_font_size_multiplier
                    * calc_merged_font_size_multiplier(count as i32),
            );
            ...
            current_processed.font_size_multiplier = sanitize_font_size_multiplier(
                current_processed.original_font_size_multiplier
                    * calc_merged_font_size_multiplier(count as i32),
            );
...
fn sanitize_font_size_multiplier(value: f64) -> f64 {
    if value.is_finite() {
        value.clamp(0.25, 8.0)
    } else {
        1.0
    }
}
...
fn calc_layout_cache_key(
    ...
    for item in items {
        ...
        item.height.to_bits().hash(&mut hasher);
...
mod tests {
    ...
    fn mk_working_scroll_item(
        ...
        WorkingNext2Item {
            ...
            y_position: 0.0,
            width,
            height: 24.0 * 1.2 * font_size_multiplier,
