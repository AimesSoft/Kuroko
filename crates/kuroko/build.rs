...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=KUROKO_NATIVE_PROFILE");
    println!("cargo:rerun-if-env-changed=KUROKO_LIBASS_DIR");
    println!("cargo:rerun-if-env-changed=KUROKO_FREETYPE_DIR");
    ...
    let libass = native_dep_dir("KUROKO_LIBASS_DIR", "libass");
    let freetype = native_dep_dir("KUROKO_FREETYPE_DIR", "freetype");
    let harfbuzz = native_dep_dir("KUROKO_HARFBUZZ_DIR", "harfbuzz");
    let fribidi = native_dep_dir("KUROKO_FRIBIDI_DIR", "fribidi");

    for dir in [&libass, &freetype, &harfbuzz, &fribidi] {
        if !dir.join("lib").exists() {
            panic!(
                "native dependency was not found at {}. Run `cargo run -p xtask -- deps build --all --profile {}` first, or set KUROKO_*_DIR.",
                dir.display(),
                native_profile()
            );
        }
...
fn native_profile() -> String {
    env::var("KUROKO_NATIVE_PROFILE").unwrap_or_else(|_| "lgpl".to_string())
}
...
fn workspace_root() -> PathBuf {
    PathBuf::from(env::var("CARGO_MANIFEST_DIR").expect("CARGO_MANIFEST_DIR is set"))
        ...
        .expect("crates/kuroko has a workspace root")
        .to_path_buf()
}
